...
public class BeanDefinitionParserDelegate {
	...
	public AbstractBeanDefinition parseBeanDefinitionAttributes(Element ele, String beanName,
		...
		/**
		 * 是否是抽象 （标签中有没有其他属性 父类的属性继承到子类当中来 ）
		 * abstract = "true/false"     Spring容器中有父子BeanDefinition的关系，基本不怎么用
		 * 实例化过程中会获取到 父子BeanDefinition在实例化过程中会合并成一个
		 */
		...
		/**
		 * 懒加载属性（懒加载的意思就是容器初始化的时候不会对beanDefinition进行实例化，只有调用getBean的时候才会实例化）
		 */
		...
		/**
		 * 不需要依赖注入就把该属性设置为FALSE（autowire-candidate）
		 */
		 ...
		 * 实例化对象的操作可以不交给Spring来做，可以自己创建工厂类，指定该工厂类的实例bean的方法
		 * 基本不会去使用到，除非是对Bean初始化有其他业务的关联
		 */
	...
	public void parseMetaElements(Element ele, BeanMetadataAttributeAccessor attributeAccessor) {
		...
		for (int i = 0; i < nl.getLength(); i++) {
			...
			if (isCandidateElement(node) && nodeNameEquals(node, META_ELEMENT)) {
				...
				// 把元数据拿出来封装成BeanMetadataAttribute对象
	 ...
	 *
	 * MethodOverrides 是对覆盖方法的包装类型集合，MethodOverride是集合中的元素。
	 * MethodOverride是LookupMethod对象的箱子
	 *
	 * BeanDefinition中包含 MethodOverrides 对象。
	 */
	public void parseLookupOverrideSubElements(Element beanEle, MethodOverrides overrides) {
		...
		for (int i = 0; i < nl.getLength(); i++) {
			...
			if (isCandidateElement(node) && nodeNameEquals(node, LOOKUP_METHOD_ELEMENT)) {
				...
				/**
				 * LookupOverride继承了MethodOverride，可以装入MethodOverrides中
				 */
	...
	public void parseReplacedMethodSubElements(Element beanEle, MethodOverrides overrides) {
		...
		for (int i = 0; i < nl.getLength(); i++) {
			...
			if (isCandidateElement(node) && nodeNameEquals(node, REPLACED_METHOD_ELEMENT)) {
				...
				/**
				 * ReplaceOverride是MethodOverride的子类
				 */
				...
				if (index < 0) {
					...
					try {
						...
						if (bd.getConstructorArgumentValues().hasIndexedArgumentValue(index)) {
							...
							/**
							 * 获取ConstructorArgumentValues对象，在其列表中追加一个constructor-args（ValueHolder）的值
							 */
