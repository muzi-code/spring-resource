...
class DisposableBeanAdapter implements DisposableBean, Runnable, Serializable {
	...
	public DisposableBeanAdapter(Object bean, String beanName, RootBeanDefinition beanDefinition,
		...
		/**
		 * 目的
		 * 1。拿到destory-method属性值方法
		 * 2。拿到beanPostProcessor
		 */
		this.bean = bean;
		this.beanName = beanName;

		/**
		 * 判断当前的bean是否实现了DisposableBean接口
		 * 实现这个接口的有个销毁的方法
		 */
		...
		/**
		 * 过滤下当前容器前所有的PostProcessors拿到DestructionAwareBeanPostProcessor类型的集合
	...
	private String inferDestroyMethodIfNecessary(Object bean, RootBeanDefinition beanDefinition) {
		/**
		 * 拿beanDefinition 的destory-method属性的内容
		 */
			...
			for (BeanPostProcessor processor : processors) {
				...
				if (processor instanceof DestructionAwareBeanPostProcessor) {
					...
					if (dabpp.requiresDestruction(bean)) {
						/**
						 * InitDestroyAnnotationBeanPostProcessor保存到集合中
						 */
	...
	/**
	 * 销毁 destroy 方法是谁来调用？  tomcat来调用的。
	 *
	 * Servlet规范监听器ContextLoaderListener实现了ServletContextListener，
	 * tomcat关掉，需要调contextDestoryed方法
	 *
	 * closeWebApplicationContext（）
	 * doClose（）
	 * destorySingletons（） 拿到disposableBeans容器就是维护的
	 * 会调用到每一个bean的destory方法。
	 *
	 * tomcat 关闭 监听器监听
	 */
	@Override
	public void destroy() {
		...
		/**
		 * 调用bean实现 DisposableBean接口的 destory方法
		 */
