...
public class ComponentScanBeanDefinitionParser implements BeanDefinitionParser {
	 ...
	 *
	 * 1.扫描路径 .class后缀的文件
	 * 2.判断类上面是否有注解，判断过程，AnnotationMetadata包装了类对方法注解等信息，includeFilter中的默认注解match Metadata信息匹配注解。
	 * 3.创建BeanDefintion 取 AnnotationMetadata 的数据补全BeanDefintion中信息。
	 * 4.注册BeanDefinition名称的列表和BeanDefinition的名称和对象的映射Map
	 * 5.注册BeanDefinition中使用到的组件（BeanPostProcessor实现类）
	 */
	@Override
	@Nullable
	 ...
	 * 约定大于配置，大家都用默认的。
	 *
	 * 工程里面太多需要配置需要去定制化选择，这样做反而不是好事，需要的要求太高了。
	 * 所以大多数框架都是提供默认的配置，并且提供定制化修改方式。
	 * 不懂详细配置也能用，想详细的配置也可以。
	 */
	...
	protected ClassPathBeanDefinitionScanner createScanner(XmlReaderContext readerContext, boolean useDefaultFilters) {
		...
		/**
		 * ClassPathBeanDefinitionScanner 这个类常用，Mybatis扫描器继承了这个类
		 */
	...
	protected void parseTypeFilters(Element element, ClassPathBeanDefinitionScanner scanner, ParserContext parserContext) {
		/**
		 *  扫描包含的注解 和 不包含的注解 include-filter  exclude-filter 属性会加入到集合中
		 */
